...
if __name__ == '__main__':
    ...
    parser.add_argument('-sr', '--split_ratio', type=float, help='(float) The train test split ratio', default='0.1')
    ...
    if args.dev:
        X, Y = load_jpg_data(df_train, train_data_dir, label_map, img_size = args.in_size, subset_size=100)
    else:
    ...
    data_dir = os.path.join('augmented',
                            'train_augmented_size_' + str(args.in_size) + '_mult_' + str(args.augmentation_multiplier) + '_seed_' + str(data_split_seed) + '_full')
    ...
    X_train_augmented, X_valid_augmented, Y_train_augmented, Y_valid_augmented = train_test_split(X_augmented, Y_augmented, test_size=args.split_ratio, random_state=data_split_seed)
    ...
    print("Saving X_valid.npz of shape %s" % str(X_valid_augmented.shape))
    np.savez_compressed(os.path.join(data_dir, 'X_valid.npz'), X_valid_augmented)
    print("Saving Y_valid.npz of shape %s" % str(Y_valid_augmented.shape))
    np.savez_compressed(os.path.join(data_dir, 'Y_valid.npz'), Y_valid_augmented)
